week_one <- read.csv("tracking_week_3.csv")
...
# removing unneeded observations
week1_removal <- week1_removal[-c(2, 3, 6, 8, 9, 11, 
                                  14, 15, 16, 18, 20,
...
# Removing rows with nulls, I believe this happens because there is not a targeted receiver on the play
# So ball would have been thrown away, no targeted receiver 
week1_final <- week1_final[-c(4, 15, 42, 47), ]


# making sure columns arent lists 
week1_final[1:11] <- lapply(week1_final[1:11], as.numeric)
week1_final[12:14] <- lapply(week1_final[12:14], as.character)

write.csv(week1_final, file = "week3.csv", row.names = TRUE)